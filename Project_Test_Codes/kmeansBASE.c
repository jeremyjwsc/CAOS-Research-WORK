...
{
	int i, j, k, min_arg, r=0;
	float dist1, dist2, min_dist=FLT_MAX;
	for(i=0; i<C; i++)
        {
	...
	for(i=0; i<N; i++)
		...
		if (C&1 == 1)
		  ...
		  for(k=0; k<D; k++)
                  {
                      float punt= Punts[i*D+k];
		      float temporal1 = punt - Centroides[j*D+k];
		      dist1 += temporal1*temporal1;
		  }
                  min_arg = (dist1<min_dist)? j    : min_arg;
                  min_dist= (dist1<min_dist)? dist1: min_dist;
                  j= 1;
                  dist1=0;
		}
		...
		for(; j<C; j+=2)
			...
			for(k=0; k<D; k++)
			   ...
			   float temporal1 = punt - Centroides[j*D+k];
			   float temporal2 = punt - Centroides[(j+1)*D+k];
			   dist1 += temporal1*temporal1;
			   dist2 += temporal2*temporal2;
			}
                     min_arg = (dist1<min_dist)? j    : min_arg;
                     min_dist= (dist1<min_dist)? dist1: min_dist;
                     min_arg = (dist2<min_dist)? j+1  : min_arg;
                     min_dist= (dist2<min_dist)? dist2: min_dist;
		}
		...
		Sep[min_arg]+=1;
		for (k=0; k<D; k++) 
		   New_Centr[min_arg*D+k] += Punts[i*D+k];
	}
