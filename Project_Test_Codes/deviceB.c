...
int PointsToCentroides(float *Punts, int N, int D, int C, float *Centroides, float *New_Centr, int *Labels, int *Sep)
{
	...
	for (i=0; i<C; i++)
	   ...
	   for (k=0; k<D; k++)
           {
	     float value = New_Centr[i*D+k] / (float)Sep[i];

	     // if centre is updated, set flag <r> to 1.
	     if (value != Centroides[(i*D)+k])
               *pR = 1;
             
	     // update with new centroid
             Centroides[i*D+k] = value;
           }
         }
     } // data present
...
int main(int argc, char **argv){
	...
	int   *Labels     = (int*)   malloc( sizeof(int)   * N * C );	// Index array of points which assigned to each centroid	
    ...
    #pragma acc data copy(Punts[N*D]) copy(Labels[N*C]) copy(Centroides[C*D]) create(New_Centr[C*D]) copyout(Sep[C])
	...
	for (i = 0; i < C; i++) {
		...
		for (j = 0; j < Sep[i]; j++) {
		  {
		    fprintf(result_file, "\t%s,", row_name[Labels[i*N + j]]);
			printf("\t%s,", row_name[Labels[i*N + j]]);
	            for (k = 0; k < D; k++) 
                    {
			fprintf(result_file, "%3.1f,", Punts[Labels[i*N+j]*D+k]);
		    }
		    fprintf(result_file, "\n");
		  }
                }
